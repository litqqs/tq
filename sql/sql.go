...
func (rs *Rows) initCols() bool {
	...
	if rs.lastcols == nil {
		cols := rs.rowsi.Columns()
		rs.lastcols = make([]driver.Value, len(cols))
		rs.mapCols = make(map[string]int, len(cols))
		for index, colName := range cols {
			rs.mapCols[colName] = index
		}
	}
	return true
}
...
func (rs *Rows) ScanValue(colIndex int, any interface{}) (err error) {
	...
	if colIndex < 0 || colIndex >= len(rs.lastcols) {
		return fmt.Errorf("Col index %d out of bound (0, %d) ", colIndex, len(rs.lastcols)-1)
	}
	err = convertAssign(any, rs.lastcols[colIndex])
	return
}

func (rs *Rows) I(colIndex int) (ret int64, err error) {
	err = rs.ScanValue(colIndex, &ret)
	if err != nil {
		ret = 0
	}
	return
...
func (rs *Rows) F(colIndex int) (ret float64, err error) {
	err = rs.ScanValue(colIndex, &ret)
	if err != nil {
		ret = 0
	}
...
func (rs *Rows) S(colIndex int) (ret string, err error) {
	err = rs.ScanValue(colIndex, &ret)
	if err != nil {
		ret = ""
	}
...
func (rs *Rows) B(colIndex int) (ret bool, err error) {
	err = rs.ScanValue(colIndex, &ret)
	if err != nil {
		ret = false
	}
...
func (rs *Rows) AsBool(colIndex int) (ret bool) {
	ret, _ = rs.B(colIndex)
	return
}

func (rs *Rows) T(colIndex int) (ret time.Time, err error) {
	err = rs.ScanValue(colIndex, &ret)
	if err != nil {
		ret = time.Unix(0, 0)
	}
...
func (rs *Rows) AsTime(colIndex int) (ret time.Time) {
	ret, _ = rs.T(colIndex)
	return
}
